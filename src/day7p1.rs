...
pub fn handle_commands(lines: Rc<RefCell<PutBack<Lines<BufReader<File>>>>>, directory_name: String) -> Vec<(String, u64)> {
    let mut accumulator: Vec<(String, u64)> = vec![];
        ...
        if let Some(line) = line {
            ...
            match command {
                Command::Forward(dir) => {
                    ...
                    for (subdir, size) in result {
                        own_size += size;
                        accumulator.push((subdir, size));
                    }
                },
                Command::Back => {
                    println!("moving back a directory (from {} with size {})", directory_name, own_size);
                    ...
                    loop {
                        ...
                        if let Some(line) = line {
                            if line.starts_with("$") {
                                ...
                                break; // this line isn't a response, it is a new command. so we just put it back and break
                            ...
                            let (size_str, _filename) = line.split(" ").collect_tuple().unwrap();
    ...
    println!("falling out bottom of function for directory {} with size {}", directory_name, own_size);
    accumulator.push((directory_name.clone(), own_size));
    return accumulator;
}
...
pub fn main() {
    ...
    if let Ok(mut lines) = reader::read_lines("./input7") {        
        ...
        let mut result = handle_commands(lines, String::from("root"));

        result.sort();
        // print!("{:#?}", result);
        let matching_items = result.into_iter().filter(|(_dir, size)| *size <= 100000).collect_vec();
        print!("{:#?}", matching_items);

        let matching_sizes = matching_items.into_iter().filter(|(_dir, size)| *size <= 100000).map(|(_dir, size)| size).collect_vec();
